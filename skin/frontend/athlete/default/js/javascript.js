...
/* ===================================================
 * petcubes.demo.js v1.0
 * ===================================================

// NOTICE!! This JS file is included for reference.
// This code is used to power all the JavaScript
// demos and examples for PetCubes
// ++++++++++++++++++++++++++++++++++++++++++*/
...
!function($) {
    ...
    $(function(){
		...
		$(document).ready(function(){
				...
				//$('#commentform, #searchform').find('input:not(input[type="submit"]), textarea').addClass('form-control margin-bottom-10');
				//$('#commentform, #searchform').find('input[type="submit"]').addClass('cs-form-button');
				
      //$('#wpsl-stores').simplebar();
			var c_mode = $('#petcubes-contact-form input[type="radio"][name="contact-mode"]:checked').val();	
			if(c_mode=='phone'){
				$('#petcubes-contact-form #cs-phone').removeAttr('disabled');
				$('#petcubes-contact-form #cs-email').attr('disabled', 'disabled');
				$('#petcubes-contact-form #cs-email').val('');
			}else if(c_mode=='email'){
				$('#petcubes-contact-form #cs-email').removeAttr('disabled');
				$('#petcubes-contact-form #cs-phone').attr('disabled', 'disabled');	
				$('#petcubes-contact-form #cs-phone').val('');
			}
			
			$(document).on('click', '#petcubes-contact-form .cs-form-button', function(event){
				event.preventDefault();
				$('#petcubes-contact-form #cs-phone, ' +
                '#petcubes-contact-form #cs-email, ' +
                '#petcubes-contact-form #cs-message').removeClass('has-error');

				var go_to = true;
				var c_mode = $('#petcubes-contact-form input[type="radio"][name="contact-mode"]:checked').val();	
				if(c_mode == 'phone' && $('#petcubes-contact-form #cs-phone').val() == ''){
					go_to = false;
					$('#petcubes-contact-form #cs-phone').focus();
					$('#petcubes-contact-form #cs-phone').addClass('has-error');
				}else if(c_mode == 'email' && isValidEmailAddress($('#petcubes-contact-form #cs-email').val()) == false){
					go_to = false;
					$('#petcubes-contact-form #cs-email').focus();
					$('#petcubes-contact-form#cs-email').addClass('has-error');
				}else if($('#petcubes-contact-form #cs-message').val() == ''){
					go_to = false;
					$('#petcubes-contact-form #cs-message').focus();
					$('#petcubes-contact-form #cs-message').addClass('has-error');
				}else{
					go_to = true;
				}
				if(go_to == true){
                    ...
                    //alert('ready to go');
					//alert(petcubes.ajaxurl);
                    var data = {
                        'action': 'petcubes_send_email',
                        'formdata': $('#petcubes-contact-form').serialize()
                    };

                    // since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
                    $.post(petcubes.ajaxurl, data, function(response) {
                        $('.cs-form-loader').hide();
                        //alert(response);
                        if(response == 'error'){
                            $('#petcubes-contact-form').parent('.cs-form-body').append('' +
                            '<p class="cs-form-msg error">' +
                            'Something went wrong. Message was not sent. Please try again.' +
                            '</p>');
                        }
                        if(response == 'success'){
                            $('#petcubes-contact-form').parent('.cs-form-body').append('' +
                            '<p class="cs-form-msg success">' +
                            'Message sent successfully!' +
                            '</p>');
                        }

                        $('.cs-form-msg').fadeOut(3000);
            ...
            //feeding calculator
            $(document).on('click', '#feeding-calculator-form .cs-form-button', function(event){
                event.preventDefault();
                var form_ = $('#feeding-calculator-form'),
                    weight_field = form_.find('#pc-current-weight'),
                    body_size_field = form_.find('#pc-body-size'),
                    body_size = body_size_field.val(),
                    current_weight = weight_field.val(),
                    result_field = form_.find('#pc-recommendation'),
                    recommendation = '',
                    error_html = '',
                    error_html_weight = '',
                    error_html_bodysize = '',
                ...
                if(current_weight == ''){
                    weight_field.focus();
                    weight_field.addClass('has-error');
                    error_html_weight = '<p style="margin-bottom: 0;"><em>Please enter current weight</em></p>';
                    goto = false;
                }else if(body_size == ''){
                    body_size_field.focus();
                    body_size_field.parents('.petcubes_select').find('.dropdown-toggle').addClass('has-error');
                    error_html_bodysize = '<p style="margin-bottom: 0;"><em>Please select body size</em></p>';
                    goto = false;
                }else{
                    body_size_field.parents('.petcubes_select').find('.dropdown-toggle').removeClass('has-error');
                    weight_field.removeClass('has-error');
                    error_html_weight = '';
                    error_html_bodysize = '';
                    goto = true;
                }

                recommendation = error_html_weight + error_html_bodysize;

                if(goto == true){
                    var food_val = Math.ceil((current_weight * body_size) * 1000);
                    if(food_val >= 1000){
                        food_val = (food_val / 1000) + 'kg';
                    }else{
                        food_val = food_val + 'g';
                    }
                    recommendation = '<p><em>Your dog should consume <strong>'+food_val+'</strong> of fresh food a day.*</p>'+
                    '<hr/><p style="margin-bottom:0;font-size:13px;">*** Please note that this is just a guide and like us humans, no two dogs are the same, so you may find that you need to adjust the amount depending on your pets activity levels and metabolism. For puppies, key in their ideal adult weight and slowly increase their meals accordingly. Always aim for gradual weight loss or gain, and seek advice from your vet when deciding on the ideal weight for your pet. We recommend feeding your pet twice daily, so you should divide the total daily feeding weight in half and feed once in the morning and once in the evening.</p>';

                }else{
                    recommendation = error_html_weight + error_html_bodysize;
                }
                /*var prev_height = result_field.height();*/
                result_field.html(recommendation);
                /*var after_height = result_field.height();
                if(after_height > prev_height){
                    result_field.append('<p><a>See more</a></p>');
                    result_field.css({'height' : prev_height, 'overflow' : 'hidden'});
		...
		$('#petcubes-contact-form input[type="radio"][name="contact-mode"]').on('change', function(){
			var c_mode = $(this).val();
			if(c_mode=='phone'){
				$('#petcubes-contact-form #cs-phone').removeAttr('disabled');
				$('#petcubes-contact-form #cs-email').attr('disabled', 'disabled');
